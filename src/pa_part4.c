...
static int ensure_parent_dir(const char *path) {
    ...
    char *last_sep = NULL;
    for (char *p = buf; *p; ++p) { if (*p == '/' || *p == '\\') last_sep = p; }
    ...
    char saved = *last_sep; *last_sep = '\0';
#ifdef _WIN32
    if (_mkdir(buf) != 0 && errno != EEXIST)
#else
    if (mkdir(buf, 0755) != 0 && errno != EEXIST)
#endif
    { if (errno != EEXIST) { fprintf(stderr, "mkdir '%s' failed (errno=%d)\n", buf, errno); *last_sep = saved; return -1; } }
    *last_sep = saved;
    return 0;
}
...
static int read_matrix(const char *path, double *mat, size_t n) {
    ...
    size_t need = n * n; size_t got = fread(mat, sizeof(double), need, f);
    if (got != need) { fprintf(stderr, "Error: expected %zu doubles, read %zu from '%s'.\n", (size_t)need, (size_t)got, path); fclose(f); return -1; }
    fclose(f); return 0;
}
...
static int write_matrix(const char *path, const double *mat, size_t n) {
    ...
    size_t need = n * n; size_t wrote = fwrite(mat, sizeof(double), need, f);
    if (wrote != need) { fprintf(stderr, "Error: expected to write %zu doubles, wrote %zu to '%s'.\n", (size_t)need, (size_t)wrote, path); fclose(f); return -1; }
    fclose(f); return 0;
}
...
static void matmul_rows(const double *a, const double *b, double *c, size_t n, size_t r0, size_t r1) {
    for (size_t i = r0; i < r1; ++i) {
        ...
        for (size_t j = 0; j < n; ++j) c[i_off + j] = 0.0; // zero this row range only
...
int main(int argc, char **argv) {
    ...
    const char *out = (argc > 3) ? argv[3] : "build/output/data3_part4";
    ...
    uintptr_t h[2];
    for (int i = 0; i < 2; ++i) {
        h[i] = _beginthreadex(NULL, 0, worker, &args[i], 0, NULL);
        if (!h[i]) { fprintf(stderr, "thread create failed\n"); return 1; }
    }
    WaitForMultipleObjects(2, (HANDLE*)h, TRUE, INFINITE);
    for (int i = 0; i < 2; ++i) CloseHandle((HANDLE)h[i]);
#else
    pthread_t th[2];
    for (int i = 0; i < 2; ++i) pthread_create(&th[i], NULL, worker, &args[i]);
