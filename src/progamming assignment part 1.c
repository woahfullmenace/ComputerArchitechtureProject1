...
// Debug printing control: set DEBUG to 1 to enable verbose prints
#ifndef DEBUG
#define DEBUG 0
#endif
...
static int ensure_parent_dir(const char *path) {
    // Create parent directories for a given path (handles nested dirs).
    ...
    // Find last separator; if none, nothing to create.
    char *last_sep = NULL;
    for (char *p = buf; *p; ++p) {
        if (*p == '/' || *p == '\\') last_sep = p;
    }
...
#endif
                    // Ignore if already exists, error otherwise.
                    if (errno != EEXIST) {
    ...
    // Create the final directory (up to last_sep)
    char saved = *(last_sep);
    *last_sep = '\0';
    if (buf[0] != '\0') {
#ifdef _WIN32
...
static void print_matrix(const char *name, const double *m, size_t n) {
    DBG_PRINT("[DEBUG] Dumping matrix %s (%zux%zu)\n", name, n, n);
    for (size_t i = 0; i < n; ++i) {
        for (size_t j = 0; j < n; ++j) {
            DBG_PRINT("%s[%zu][%zu]=%.6f%s", name, i, j, m[i * n + j], (j == n - 1) ? "\n" : " ");
        }
    }
}
...
static void matmul(const double *a, const double *b, double *c, size_t n) {
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (DEBUG && (i % 100 == 0)) {
            DBG_PRINT("[DEBUG] matmul progress: row %zu/%zu\n", (size_t)i, n);
        }
    }
}
...
int main(int argc, char **argv) {
    ...
    DBG_PRINT("[DEBUG] Begin read inputs\n");
    ...
    DBG_PRINT("[DEBUG] Input matrices loaded\n");
    print_matrix("mat1", mat1, N);
    print_matrix("mat2", mat2, N);

    DBG_PRINT("[DEBUG] Starting matrix multiply...\n");
    double t0 = now_seconds();
    matmul(mat1, mat2, mat3, N);
    double t1 = now_seconds();
    double elapsed = t1 - t0;
    DBG_PRINT("[DEBUG] Matrix multiply completed.\n");
    print_matrix("mat3", mat3, N);

    DBG_PRINT("[DEBUG] Writing output matrix...\n");
    ...
    DBG_PRINT("[DEBUG] Output written to '%s'\n", out);

    printf("%lf %lf %lf %lf\n", mat3[6 * N + 0], mat3[5 * N + 3], mat3[5 * N + 4], mat3[901 * N + 7]);
    fprintf(stderr, "Multiply time: %.6f seconds\n", elapsed);
